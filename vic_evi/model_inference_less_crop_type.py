...
if __name__ == "__main__":
    ...
    #### remove some types remove 'Chickpea' 'Lentils' #######
    crop_mask = ((y_1819 == 0) | (y_1819 == 1) | (y_1819 == 4))

    # #### only keep 'Chickpea' 'Lentils' #######
    # crop_mask = ((y_1819 == 2) | (y_1819 == 3))

    X_1819 = X_1819[crop_mask]
    y_1819 = y_1819[crop_mask]
    y_DegreeD_1819 = y_DegreeD_1819[crop_mask]
    y_Rain_1819 = y_Rain_1819[crop_mask]
    ...
    predictions = np.vstack(list(map(
        ...
        runner.predict_loader(model=model,
                              loader=test_dl, resume=f"{logdir}/model/nonecrop/chickpea_lentils.pth")
    )))
    ...
    results.columns = ['Prob_Barley', 'Prob_Canola',
                       'Prob_Wheat', 'Pred_label', 'True_label', 'Pred_class']
    # classification report

    y_true = true_labels
    y_pred = pred_labels
    # target_names = ['Barley', 'Canola', 'Chickpea', 'Lentils', 'Wheat']
    target_names = ['Barley', 'Canola', 'Wheat']
    ...
    # results.to_csv(f"{logdir}/predictions/predictions_nonecrop.csv", index=False)
