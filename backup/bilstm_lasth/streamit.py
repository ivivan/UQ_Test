...
if __name__ == "__main__":
    ...
    if uploaded_file is not None:
        st.markdown("Show NDVI Curve")
        data_plot = data.T.to_numpy()
        st.write(data_plot)
        st.line_chart(data_plot)

    
    if uploaded_file is not None:
        st.markdown("Prepare Model Input")
        ...
        # valid_ds = TensorDataset(X_test_tensor, X_test_tensor)
    ...
    # #                           loader=test_dl, resume="backup/bilstm_lasth/best_full.pth")
    # # )))
