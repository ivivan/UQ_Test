...
def preprocess_df(df):
    ...
    df.drop(['chloro_con_x'], axis=1, inplace=True)
    df.drop(['chloro_con_y'], axis=1, inplace=True)

    tw = df['temp_water_x'].values.copy().reshape(-1, 1)

    # Standlization, use StandardScaler
    scaler_x = MinMaxScaler()
    scaler_x.fit(
        df[['temp_water_x', 'ph_x', 'spec_cond_x', 'diss_oxy_con_x', 'nitrate_con_x','temp_water_y', 'ph_y', 'spec_cond_y', 'diss_oxy_con_y', 'nitrate_con_y']])
    df[['temp_water_x', 'ph_x', 'spec_cond_x', 'diss_oxy_con_x', 'nitrate_con_x','temp_water_y', 'ph_y', 'spec_cond_y', 'diss_oxy_con_y', 'nitrate_con_y']] = scaler_x.transform(df[[
            'temp_water_x', 'ph_x', 'spec_cond_x', 'diss_oxy_con_x', 'nitrate_con_x','temp_water_y', 'ph_y', 'spec_cond_y', 'diss_oxy_con_y', 'nitrate_con_y'
        ]])
    ...
    # get data from 2017 and 2018
    # 1-11 as train; 12 as test

    df_train_one = df.loc['01/04/2017 00:00':'30/11/2017 23:30'].copy()
    df_train_two = df.loc['01/01/2018 00:00':'30/11/2018 23:30'].copy()

    df_test_one = df.loc['01/12/2017 00:00':'31/12/2017 23:30'].copy()
    df_test_two = df.loc['01/12/2018 00:00':'31/12/2018 23:30'].copy()
...
def train_val_test_generate(dataframe, model_params):
    ...
    train_val_test_x, train_val_test_y, len_x_samples, len_before_x_samples = pad_all_cases(
        dataframe, dataframe['temp_water_x'].values, model_params,
...
def train_test_split_SSIM(x, y, x_len, x_before_len, model_params, SEED):
    ...
    y_train = y
    x_train_len = x_len
    x_train_before_len = x_before_len
...
def test_pm25_single_station():
    ...
    filepath = r'data\Iowa\two_year.csv'
